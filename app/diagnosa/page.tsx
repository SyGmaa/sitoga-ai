...
export default function DiagnosaPage() {
  ...
  const renderDiagnosisCard = (toolCallId: string, data: any) => {
    ...
    return (
      <div key={toolCallId} className="flex flex-col gap-4 w-full">
        ...
        <div className="bg-white dark:bg-surface-dark/80 text-left rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-primary/20 w-full animate-[fadeIn_0.5s_ease-out]">
          <div className="bg-slate-50 dark:bg-[#234829]/30 px-6 py-4 border-b border-slate-100 dark:border-[#234829] flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="bg-primary/20 p-2 rounded-lg text-primary">
                <span className="material-symbols-outlined">medical_services</span>
              </div>
              <div>
                <h3 className="text-slate-800 dark:text-white font-bold text-lg">Potential Condition</h3>
              </div>
            </div>
          </div>
          <div className="p-6">
            <h4 className="text-xl text-slate-800 dark:text-white font-semibold mb-2">{data.nama_penyakit}</h4>
            
            {gejalaClean.length > 0 && (
              <p className="text-slate-600 dark:text-slate-300 text-sm mb-4">
                Gejala terdeteksi: <span className="text-[#3a7544] dark:text-[#92c99b]">{gejalaClean.join(", ")}</span>
              </p>
            )}
            
            {rekomendasiClean.length > 0 ? (
              <div className="space-y-4">
                <h5 className="text-[#3a7544] dark:text-[#92c99b] text-xs font-bold uppercase tracking-wider mb-2">Rekomendasi Tanaman</h5>
                {rekomendasiClean.map((plant: any, idx: number) => (
                  <div key={idx} className="group flex items-center gap-4 p-3 rounded-xl bg-slate-50 dark:bg-[#234829]/20 border border-slate-200 dark:border-transparent hover:border-primary/40 dark:hover:border-primary/40 hover:bg-slate-100 dark:hover:bg-[#234829]/40 transition-all cursor-pointer">
                    <div className="flex-1">
                      <h6 className="text-slate-800 dark:text-white font-bold">{plant.nama}</h6>
                      <p className="text-slate-500 dark:text-slate-400 text-xs italic">View full details to see instructions.</p>
                    </div>
                    <Link href={`/tanaman/${plant.id}`} className="material-symbols-outlined text-slate-400 dark:text-slate-500 group-hover:text-primary transition-colors">chevron_right</Link>
                  </div>
                ))}
              </div>
            ) : (
  ...
  const renderRujukDokter = (toolCallId: string, data: any) => {
    return (
      <div key={toolCallId} className="bg-white dark:bg-surface-dark/80 text-left rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-red-500/30 w-full animate-[fadeIn_0.5s_ease-out]">
        <div className="bg-red-50 dark:bg-red-900/30 px-6 py-4 border-b border-red-100 dark:border-red-500/30 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-red-500/20 p-2 rounded-lg text-red-500 dark:text-red-400">
              <span className="material-symbols-outlined">warning</span>
            </div>
            <div>
              <h3 className="text-slate-800 dark:text-white font-bold text-lg">Peringatan Medis</h3>
            </div>
          </div>
        </div>
        <div className="p-6">
          ...
          <div className="p-4 bg-red-50/55 dark:bg-red-950/50 rounded-lg border border-red-100 dark:border-red-500/20">
            <p className="text-red-700 dark:text-red-300 text-sm font-semibold">{data.alasan}</p>
          </div>
    ...
    <div className="flex h-screen overflow-hidden bg-background-light dark:bg-background-dark font-sans text-slate-900 dark:text-slate-100 selection:bg-primary selection:text-background-dark">
      <nav className="hidden md:flex w-[280px] flex-col border-r border-slate-200 dark:border-[#234829]/50 bg-[#ebf1ec] dark:bg-background-dark h-full relative z-20">
        <div className="p-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-slate-800 dark:text-white text-lg font-bold tracking-tight">HerbalAI</h1>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_rgba(19,236,55,0.6)]"></span>
              <span className="text-[#3a7544] dark:text-[#92c99b] text-xs font-medium">Online</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1">
          <button onClick={() => setMessages([{ id: "sys1", role: "assistant", parts: [{ type: 'text', text: "Yooo... aku clonenya gama, siap membantu mendiagnosa penyakit kamu dengan gacorrr." }] }])} className="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#234829]/10 dark:hover:bg-[#234829]/20 text-[#3a7544] dark:text-[#92c99b] transition-colors">
        ...
        <div className="px-4 pb-4 border-t border-slate-200 dark:border-[#234829]/50 pt-4">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2 block px-1">AI Model</label>
      ...
      <main className="flex-1 flex flex-col h-full relative bg-background-light dark:bg-background-dark bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#d4e2d6] dark:from-[#3c4f3f] via-background-light dark:via-background-dark to-background-light dark:to-background-dark">
        {/* Mobile Header */}
        <header className="md:hidden flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-[#234829]/50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md sticky top-0 z-30">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => setShowModelPicker(!showModelPicker)}
              disabled={isModelsLoading || aiModels.length === 0}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-[#234829]/50 border border-slate-200 dark:border-[#234829] text-xs text-slate-800 dark:text-slate-300 hover:border-primary/40 transition-colors disabled:opacity-50"
            >
        ...
        {showModelPicker && (
          <div className="md:hidden border-b border-slate-200 dark:border-[#234829]/50 bg-white dark:bg-surface-dark/95 backdrop-blur-md z-20 animate-[fadeIn_0.15s_ease-out]">
            {aiModels.map((m, idx) => (
              <button
                key={m.id}
                onClick={() => { setSelectedModelIdx(idx); setShowModelPicker(false); }}
                ...
                <div className={`flex flex-col gap-2 max-w-[85%] lg:max-w-[70%] ${m.role === 'user' ? 'items-end' : ''}`}>
                  ...
                  {m.role === 'assistant' && index === 0 && (
                    <div className="flex gap-2 flex-wrap mt-1">
                      <button onClick={(e) => handleSendMessage(e, "Sakit kepala dan kurang tidur")} className="px-4 py-2 rounded-full border border-slate-200 dark:border-[#234829] bg-transparent text-sm text-slate-600 dark:text-[#92c99b] hover:border-primary hover:text-primary transition-colors flex items-center gap-2">
                        <span className="material-symbols-outlined text-[18px]">sick</span>
                        Sakit kepala
                      </button>
                      <button onClick={(e) => handleSendMessage(e, "Saya merasa mual dan perut perih sejak pagi")} className="px-4 py-2 rounded-full border border-slate-200 dark:border-[#234829] bg-transparent text-sm text-slate-600 dark:text-[#92c99b] hover:border-primary hover:text-primary transition-colors flex items-center gap-2">
