  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col group/design-root">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <section className="py-24 px-6 md:px-10 bg-background-light dark:bg-background-dark relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {plants.map((plant) => {
                ...
                <Link key={plant.id} href={`/tanaman/${plant.id}`} className="group relative rounded-3xl overflow-hidden glass-card hover:border-primary/50 transition-all duration-300 hover:-translate-y-2 cursor-pointer h-full flex flex-col">
                  <div className="aspect-[4/3] w-full overflow-hidden bg-slate-100 dark:bg-surface-dark relative">
                    {plant.gambarUrl ? (
                      <img 
                        alt={plant.namaLokal} 
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="text-2xl font-bold text-slate-800 dark:text-white group-hover:text-primary transition-colors">{plant.namaLokal}</h3>
                    ...
                    <p className="text-slate-500 dark:text-slate-300 italic text-sm mb-4">{plant.namaLatin}</p>
                    <div className="mt-auto flex flex-wrap gap-2">
                       {tags.map((tag, idx) => (
                         <span key={idx} className={`px-3 py-1 rounded-full ${idx === 0 ? 'bg-slate-100 dark:bg-surface-dark border border-slate-200 dark:border-white/10 text-xs text-primary font-medium' : 'bg-slate-100 dark:bg-surface-dark border border-slate-200 dark:border-white/10 text-xs text-slate-500 dark:text-slate-300 font-medium'}`}>
                           {tag}
                         </span>
                       ))}
                    </div>
